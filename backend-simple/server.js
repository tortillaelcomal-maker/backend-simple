...
async function initializeData() {
  try {
    ...
    const TIENDAS = [
      ...
      'ANEXO', 'GUERA', 'CHUNDE', 'HERNANDEZ', 'JUANJO', 'ALAN', 
...
app.post("/api/registros/save-ruta", auth, async (req, res) => {
  try {
    ...
    for (const [tiendaNombre, tiendaData] of Object.entries(datos)) {
      try {
        // Normalizar nombre de tienda (trim y uppercase para consistencia)
        const tiendaNormalizada = tiendaNombre.trim().toUpperCase();
        
        await Registro.findOneAndUpdate(
          { tienda: tiendaNormalizada, fecha: new Date(fecha) },
          {
        ...
        savedCount.push(tiendaNormalizada)
      } catch (error) {
...
// Migrar nombres de tiendas para normalizar
app.post("/api/registros/normalizar-tiendas", auth, async (req, res) => {
  try {
    const registros = await Registro.find({});
    let updated = 0;
    
    for (const registro of registros) {
      const tiendaNormalizada = registro.tienda.trim().toUpperCase();
      if (registro.tienda !== tiendaNormalizada) {
        await Registro.findByIdAndUpdate(registro._id, { tienda: tiendaNormalizada });
        updated++;
      }
    }
    
    res.json({ success: true, message: `${updated} registros normalizados` });
  } catch (error) {
    res.status(500).json({ success: false, message: "Error normalizando tiendas" });
  }
});

// Actualizar nombre de tienda específica
app.post("/api/tiendas/actualizar-nombre", auth, async (req, res) => {
  try {
    const { nombreViejo, nombreNuevo } = req.body;
    
    if (!nombreViejo || !nombreNuevo) {
      return res.status(400).json({ success: false, message: "Nombre viejo y nuevo son requeridos" });
    }
    
    // Actualizar en la colección de tiendas
    const tiendaActualizada = await Tienda.findOneAndUpdate(
      { nombre: nombreViejo.trim().toUpperCase() },
      { nombre: nombreNuevo.trim().toUpperCase() },
      { new: true }
    );
    
    // Actualizar todos los registros existentes
    const registrosActualizados = await Registro.updateMany(
      { tienda: nombreViejo.trim().toUpperCase() },
      { tienda: nombreNuevo.trim().toUpperCase() }
    );
    
    res.json({ 
      success: true, 
      message: `Tienda actualizada: ${nombreViejo} → ${nombreNuevo}`,
      tiendaActualizada: !!tiendaActualizada,
      registrosActualizados: registrosActualizados.modifiedCount
    });
  } catch (error) {
