...
app.get("/api/registros/reporte-dia", auth, async (req, res) => {
  try {
    ...
    console.log(`=== REPORTE DEBUG ${fecha} ===`)
    console.log(`Registros encontrados: ${registros.length}`)
    registros.forEach(r => {
      console.log(`${r.tienda}: kilos=${r.kilos}, frijoles=${r.frijoles}, observaciones=${r.observaciones?.substring(0,50)}`)
    })
    ...
    registros.forEach(r => {
      ...
      if (hasData) {
        // Special handling for GORDITAS - exclude kilos from totals (per user request)
        if (r.tienda === 'GORDITAS') {
          // For GORDITAS: only count frijoles in totals, NOT kilos
          ...
          // NOTE: GORDITAS kilos are NOT added to totalKilos
        } else {
